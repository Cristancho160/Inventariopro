...
const ProductManagementInterface = () => {
  ...
  const getNumericValue = (value) => {
    // Si el valor es nulo o cadena vacía, retorna 0. De lo contrario, retorna el número.
    return (value === null || value === '') ? 0 : parseFloat(value);
  };

  const handleSaveProduct = async (productData) => {
    // 1. Mapeo de datos (DENTRO de la función)
    const dataToSend = {
      name: productData.name,
      sku: productData.sku,
      quantity: parseInt(productData.stock, 10) || 0,
      price: getNumericValue(productData.price),
      description: productData.description,
      brand: productData.brand,
      category: productData.category,
      cost: getNumericValue(productData.cost),
      min_stock: parseInt(productData.minStock, 10) || 0,
      supplier: productData.supplier,
      barcode: productData.barcode,
      weight: getNumericValue(productData.weight),
      dimensions: productData.dimensions,
      status: productData.status
    };

    // 2. Llamada a la API (DENTRO de la función)
    const API_URL = import.meta.env.VITE_API_URL || 'http://localhost/inventorypro_api/api';

    try {
      const response = await axios.post(`${API_URL}/add_item.php`, dataToSend, {
        headers: {
      ...
      if (response.data && response.data.message === 'Item added successfully') {
        ...
        // Si el guardado es exitoso, aquí recargarías los productos
      } else {
        console.error('Error al guardar el producto:', response.data.error || 'Respuesta inesperada');
      }

    } catch (error) {
      console.error('Error de conexión con la API o de red:', error);
    } finally {
      // 3. Cierra el modal y restablece el estado
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className={`transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-80'
        } pt-16`}>
        <div className="p-6">
          <Breadcrumb />
